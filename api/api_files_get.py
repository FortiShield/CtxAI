        ...
        try:
            ...
            for path in paths:
                try:
                    ...
                    if path.startswith("/ctx0/tmp/uploads/"):
                        # Internal path - convert to external
                        filename = path.replace("/ctx0/tmp/uploads/", "")
                        external_path = files.get_abs_path("usr/uploads", filename)
                        filename = os.path.basename(external_path)
                    elif path.startswith("/ctx0/"):
                        # Other internal CtxAI paths
                        relative_path = path.replace("/ctx0/", "")
