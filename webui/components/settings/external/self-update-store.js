...
const model = {
  ...
  updateProgressOverlay() {
    ...
    if (detail) {
      detail.textContent =
        this.restartDetailText ||
        "CtxAI is restarting, applying the requested release, and will reload this page when the health check responds again.";
    }
  },
  ...
  async scheduleUpdate() {
    ...
    this.setRestartState(
      "Preparing update",
      "Saving the request and asking CtxAI to restart."
    );
    this.ensureProgressOverlay();
    try {
      ...
      notificationStore.frontendWarning(
        "CtxAI is restarting to apply the requested branch and version target.",
        "Self Update",
        10,
        "self-update-restart",
        undefined,
  ...
  async restartAndReload() {
    ...
    this.setRestartState(
      ...
      "The request was saved. CtxAI is about to restart and apply the requested branch and version target."
    );
    this.ensureProgressOverlay();
    ...
    try {
      ...
      if (restartResponse && !restartResponse.ok) {
        if (restartResponse.status >= 500) {
          console.warn(
            `Restart request returned HTTP ${restartResponse.status} while CtxAI was shutting down. Continuing to wait for the new runtime.`
          );
          this.setRestartState(
            "Restarting backend",
            "CtxAI is shutting down and applying the update. Waiting for the new runtime to come back healthy."
          );
        } else {
          throw new Error(
        ...
        this.setRestartState(
          "Restarting backend",
          "CtxAI accepted the restart request. Waiting for the updater to take over."
        );
      }
    } catch (error) {
      ...
      console.warn(
        "Restart request connection closed while CtxAI was restarting:",
        error
      );
    }
    ...
    this.setRestartState(
      "Update in progress",
      "CtxAI is restarting and the updater is running. This page will reload automatically when /api/health starts responding again."
    );
    ...
    while (Date.now() < deadline) {
      try {
        ...
        if (response.ok) {
          this.setRestartState(
            "Restarting backend",
            "Waiting for CtxAI to disconnect before reloading the page."
          );
          ...
          this.setRestartState(
            "Update in progress",
            "CtxAI is restarting and the updater is running. This page will reload automatically when the health check becomes healthy again."
          );
        ...
        this.setRestartState(
          "Update in progress",
          "CtxAI is temporarily unavailable while it restarts. Waiting for the new runtime to become healthy."
        );
    ...
    this.removeProgressOverlay();
    this.error =
      "CtxAI did not come back within the expected window. It may still be rolling back. " +
