...
from ctxai.core.agent import AgentContext, UserMessage
from ctxai.core.initialize import initialize_agent
from ctxai.utils import guids, projects
from ctxai.utils.defer import DeferredTask
from ctxai.utils.files import get_abs_path, make_dirs, read_file, write_file
from ctxai.utils.localization import Localization
from ctxai.utils.persist_chat import save_tmp_chat
...
class SchedulerTaskList(BaseModel):
    tasks: list[Annotated[ScheduledTask | AdHocTask | PlannedTask, Field(discriminator="type")]] = (
    ...
    async def add_task(self, task: ScheduledTask | AdHocTask | PlannedTask) -> "SchedulerTaskList":
        with self._lock:
    ...
    async def update_task_by_uuid(
        ...
        verify_func: Callable[[ScheduledTask | AdHocTask | PlannedTask], bool] = lambda task: True,
    ...
    async def get_due_tasks(self) -> list[ScheduledTask | AdHocTask | PlannedTask]:
        with self._lock:
            await self.reload()
            return [
                task
    ...
    def get_task_by_uuid(self, task_uuid: str) -> ScheduledTask | AdHocTask | PlannedTask | None:
        with self._lock:
...
class TaskScheduler:
    ...
    async def __new_context(self, task: ScheduledTask | AdHocTask | PlannedTask) -> AgentContext:
    ...
    async def _run_task(
        self, task: ScheduledTask | AdHocTask | PlannedTask, task_context: str | None = None
    ):

        async def _run_task_wrapper(task_uuid: str, task_context: str | None = None):

            # preflight checks with a snapshot of the task
            task_snapshot: ScheduledTask | AdHocTask | PlannedTask | None = self.get_task_by_uuid(
                task_uuid
            )
...
def serialize_tasks(tasks: list[ScheduledTask | AdHocTask | PlannedTask]) -> list[dict[str, Any]]:
    """
