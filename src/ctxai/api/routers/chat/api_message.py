...
from ctxai.core.agent import AgentContext, AgentContextType, UserMessage
from ctxai.core.initialize import initialize_agent
from ctxai.core.security import safe_filename
...
from ctxai.utils.projects import activate_project
...
class ApiMessage(ApiHandler):
    ...
    async def process(self, input: dict, request: Request) -> dict | Response:
        ...
        if not message:
            return Response(
                '{"error": "Message is required"}', status=400, mimetype="application/json"
            )
