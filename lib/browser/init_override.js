...
//     if (out.name.startsWith("data-") && out.name != "data-CTXUID" && out.name != "data-ctx-frame-id") return null;
...
//   window.__CTX_extractFrameContent = function() {
...
//           node.setAttribute("data-CTXUID", uid);
//         }
...
//         if (tagName) {
//           content += ` selector="${node.getAttribute("data-CTXUID")}"`;
//         }
...
//     if (event.data === 'CTX_REQUEST_CONTENT') {
//       // Extract content and send it back to parent
//       const content = window.__CTX_extractFrameContent();
//       // Use '*' as targetOrigin since we're in a controlled environment
//       window.parent.postMessage({
//         type: 'CTX_FRAME_CONTENT',
//         content: content,
//         frameId: window.frameElement?.getAttribute('data-ctx-frame-id')
//       }, '*');
//     }
//   });

//   // Function to extract content from all frames
//   window.__CTX_extractAllFramesContent = async function(rootNode = document) {
//     let content = "";
    
//     // Extract content from current document
//     content += window.__CTX_extractFrameContent();
...
//         iframe.setAttribute('data-ctx-frame-id', frameId);
        
//         // Setup one-time message listener for this specific frame
//         const listener = function(event) {
//           if (event.data?.type === 'CTX_FRAME_CONTENT' && 
...
//         iframe.contentWindow.postMessage('CTX_REQUEST_CONTENT', '*');
...
//       const frameId = iframe.getAttribute('data-ctx-frame-id');
