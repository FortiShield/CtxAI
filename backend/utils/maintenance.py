...
def clean_disk_space(threshold=90, max_age_days=7, dry_run=False):
            ...
            for name in fnames:
                ...
                try:
                    ...
                    if age_days > max_age_days:
                        ...
                        if dry_run:
                            PrintStyle.debug(f"[Dry Run] Would delete: {file_path} ({size} bytes, {age_days:.1f} days old)")
                        else:
                            os.remove(file_path)
    ...
    if not dry_run:
        PrintStyle.success(f"Cleanup finished. Deleted {total_deleted} files, total size: {total_size / (1024*1024):.2f} MB")
    else:
        PrintStyle.info(f"Dry run finished. Would delete {total_deleted} files, total size: {total_size / (1024*1024):.2f} MB")
...
def detect_language_simple(text):
    ...
    if counts['latin'] > total_alphabetic * 0.5:
        ...
        common_en = {' the ', ' is ', ' and ', ' of ', ' to ', ' in ', ' that ', ' it ', ' with ', ' as '}
        ...
        common_es = {' el ', ' la ', ' de ', ' que ', ' y ', ' en ', ' un ', ' ser ', ' se ', ' no '}
        es_matches = sum(1 for word in common_es if word in lower_text)
        if es_matches >= 2:
            return "es"

        common_fr = {' le ', ' la ', ' de ', ' et ', ' que ', ' un ', ' dans ', ' est ', ' ce ', ' il '}
