...
OFFICIAL_REPO_URL = os.environ.get(
    "CTX0_SELF_UPDATE_REMOTE_URL",
    "https://github.com/ctxos/ctxai.git",
)
REPO_DIR = Path("/ctx0")
TRIGGER_FILE = Path("/exe/ctx0-self-update.yaml")
STATUS_FILE = Path("/exe/ctx0-self-update-status.yaml")
LOG_FILE = Path("/exe/ctx0-self-update.log")
...
class AttemptLogger:
    ...
    def log(self, message: str = "") -> None:
        ...
        print(f"[ctx0-self-update] {message}", flush=True)
...
def create_usr_backup(
    *,
    ...
    destination_name = sanitize_filename(backup_name, "ctxai-usr-backup.zip")
...
def create_rollback_stash(repo_dir: Path, logger: AttemptLogger) -> str | None:
    ...
    message = f"ctx0-self-update rollback snapshot {now_iso()}"
    run_command(
        [
            "git",
            "-C",
...
def fetch_release_refs(repo_dir: Path, branch: str, tag: str, logger: AttemptLogger) -> None:
    remote_branch_ref = f"refs/remotes/ctx0-self-update/{branch}"
    ...
    try:
        ...
        if bool(request_data.get("backup_usr", True)):
            backup_destination = create_usr_backup(
                ...
                backup_name=str(request_data.get("backup_name", "ctxai-usr-backup.zip")),
        ...
        if healthy:
            record_result(
                status="success",
                message=f"Updated CtxAI to branch {branch}, {resolved_target['target_description']}.",
...
def trigger_update_command(args: list[str]) -> int:
    parser = argparse.ArgumentParser(
        ...
        description="Queue an CtxAI self-update for the next startup attempt.",
    )
    parser.add_argument(
        "branch",
        nargs="?",
    ...
    print("Queued CtxAI self-update for the next startup attempt.")
    ...
    print("Restart the container or CtxAI process to apply it.")
    return 0
