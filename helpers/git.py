...
# Files to ignore when checking dirty status (CTX0 project metadata)
CTX0_IGNORE_PATTERNS = {".ctx0proj", ".ctx0proj/"}


def get_repo_status(repo_path: str) -> dict:
    """Get Git repository status, ignoring CTX0 project metadata files."""
    try:
        repo = Repo(repo_path)
        if repo.bare:
        ...
        # Check dirty status, excluding CTX0 metadata
        def is_ctx0_file(path: str) -> bool:
            return path.startswith(".ctx0proj") or path == ".ctx0proj"
        
        # Filter out CTX0 files from diff and untracked
        changed_files = [d.a_path for d in repo.index.diff(None)] + [d.a_path for d in repo.index.diff("HEAD")]
        untracked = repo.untracked_files
        
        real_changes = [f for f in changed_files if not is_ctx0_file(f)]
        real_untracked = [f for f in untracked if not is_ctx0_file(f)]
