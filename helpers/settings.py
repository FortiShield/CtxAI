...
def get_default_value(name: str, value: T) -> T:
    """
    Load setting value from .env with CTX0_SET_ prefix, falling back to default.

    Args:
        name: Setting name (will be prefixed with CTX0_SET_)
    ...
    env_value = dotenv.get_dotenv_value(
        f"CTX0_SET_{name}", dotenv.get_dotenv_value(f"CTX0_SET_{name.upper()}", None)
    )
    ...
    try:
        ...
        PrintStyle(background_color="yellow", font_color="black").print(
            f"Warning: Invalid value for CTX0_SET_{name}='{env_value}': {e}. Using default: {value}"
        )
        return value
