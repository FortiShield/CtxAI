...
class VariablesPlugin(ABC):
    @abstractmethod
    def get_variables(self, file: str, backup_dirs: Union[list[str], None] = None, **kwargs) -> dict[str, Any]:  # type: ignore
        pass


def load_plugin_variables(
    file: str, backup_dirs: Union[list[str], None] = None, **kwargs
) -> dict[str, Any]:
...
def parse_file(
    _filename: str, _directories: Union[list[str], None] = None, _encoding="utf-8", **kwargs
):
...
def read_prompt_file(
    _file: str, _directories: Union[list[str], None] = None, _encoding="utf-8", **kwargs
):
    if _directories is None:
        _directories = []
...
def get_abs_path_dockerized(*relative_paths):
    "Ensures the abs path is dockerized (i.e. /ctx0/... path)"
    abs = get_abs_path(*relative_paths)
    from helpers import runtime

    if runtime.is_dockerized():
        return abs
    return normalize_ctx0_path(abs)
...
def fix_dev_path(path: str):
    "On dev environment, convert /ctx0/... paths to local absolute paths"
    from helpers.runtime import is_development

    if is_development():
        if path.startswith("/ctx0/"):
            path = path.replace("/ctx0/", "")
    return get_abs_path(path)


def normalize_ctx0_path(path: str):
    "Convert absolute paths into /ctx0/... paths"
