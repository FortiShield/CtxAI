...
OFFICIAL_REPO_AUTHOR = "ctxos"
OFFICIAL_REPO_NAME = "ctxai"
BRANCH_OPTIONS = [
...
UPDATE_FILE_PATH = Path("/exe/ctx0-self-update.yaml")
STATUS_FILE_PATH = Path("/exe/ctx0-self-update-status.yaml")
LOG_FILE_PATH = Path("/exe/ctx0-self-update.log")
...
def _get_remote_branch_merged_tags(branch: str) -> set[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="ctx0-self-update-tags-") as temp_dir:
...
def _get_remote_branch_head_info(branch: str) -> dict[str, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="ctx0-self-update-head-") as temp_dir:
        repository = Path(temp_dir)
        _run_git(repository, "init", "--bare")
        _run_git(
            repository,
