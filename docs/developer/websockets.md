...
| `handlerId` | Manager Envelope | Fully-qualified Python class name (e.g., `backend.websocket_handlers.notifications.NotificationHandler`). Used for result aggregation and logging. |
...
```json
{
  "handlerId": "backend.websocket_handlers.notifications.NotificationHandler",
...
| `HARNESS_UNKNOWN_EVENT` | Developer harness | Harness test handler received an unsupported event name. | Update harness sources or disable the step before running automation. | `{ "handlerId": "backend.websocket_handlers.dev_websocket_test_handler.DevWebsocketTestHandler", "ok": false, "error": { "code": "HARNESS_UNKNOWN_EVENT", "error": "Unhandled event", "details": "ws_tester_foo" } }` |
