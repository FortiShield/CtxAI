...
In Docker deployments, `hooks.py` normally affects the framework runtime at `/opt/venv-ctx0`, while the agent execution runtime is `/opt/venv`.
...
The **Plugin Index** is a community-maintained repository at https://github.com/ctxos/ctx-plugins. Plugins listed there are discoverable by all CtxAI users.
...
Plugin UIs must use the **CTX0 notification system** for user feedback. Do not show errors or success via inline elements (e.g. a red box bound to `store.error`).
