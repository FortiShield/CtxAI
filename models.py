...
class LiteLLMChatWrapper(SimpleChatModel):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Set CTX0 model config as instance attribute after parent init
        self.ctx0_model_conf = model_config
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        apply_rate_limiter_sync(self.ctx0_model_conf, str(msgs))
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        # Apply rate limiting if configured
        await apply_rate_limiter(self.ctx0_model_conf, str(msgs))
    ...
    async def unified_call(
        ...
        **kwargs: Any,
        ...
        limiter = await apply_rate_limiter(
            self.ctx0_model_conf, str(msgs_conv), rate_limiter_callback
        )

        # Prepare call kwargs and retry config (strip CTX0-only params before calling LiteLLM)
        call_kwargs: dict[str, Any] = {**self.kwargs, **kwargs}
        max_retries: int = int(call_kwargs.pop("ctx0_retry_attempts", 2))
        retry_delay_s: float = float(call_kwargs.pop("ctx0_retry_delay_seconds", 1.5))
...
class LiteLLMEmbeddingWrapper(Embeddings):
    model_name: str
    kwargs: dict = {}
    ctx0_model_conf: Optional[ModelConfig] = None
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        # Apply rate limiting if configured
        apply_rate_limiter_sync(self.ctx0_model_conf, " ".join(texts))
    ...
    def embed_query(self, text: str) -> List[float]:
        # Apply rate limiting if configured
        apply_rate_limiter_sync(self.ctx0_model_conf, text)
