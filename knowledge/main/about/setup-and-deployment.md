# CtxAI - Setup and Deployment

## Docker Deployment (Standard)

CtxAI is distributed as a Docker image: `ctxos/ctxai`.
...
All user data lives in `/ctx0/usr/` inside the container. Without a volume mount, data is lost when the container is removed.

Map `/ctx0/usr` to a host directory for persistence:
```bash
docker run -p 50001:80 -v /path/on/host:/ctx0/usr ctxos/ctxai
```

Contents of `/ctx0/usr/`:
...
2. Pull the new image: `docker pull ctxos/ctxai`
...
CtxAI is a Progressive Web App (PWA). On mobile, open the web UI URL in a browser, then add to home screen for an app-like experience. Works with both local network and tunnel URLs.
...
```bash
git clone https://github.com/ctxos/ctxai
cd ctxai
python -m venv .venv
