# CtxAI - Configuration Reference

## LLM Roles

CtxAI uses three configurable LLM roles:
...
- CtxAI API (ctx0_venice) - hosted service with no API key required for basic use
...
Any setting can be set via environment variable using the `CTX0_SET_` prefix. This is the primary mechanism for automated deployment and container configuration.

Format: `CTX0_SET_<setting_name>=<value>`

Examples:
```
CTX0_SET_chat_model_provider=anthropic
CTX0_SET_chat_model_name=claude-sonnet-4-5
CTX0_SET_utility_model_provider=openai
CTX0_SET_utility_model_name=gpt-4o-mini
CTX0_SET_embedding_model_provider=openai
CTX0_SET_embedding_model_name=text-embedding-3-small
```
