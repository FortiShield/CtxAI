...
- `/opt/venv-ctx0` (Python 3.12) - the CtxAI framework runtime
- `/opt/venv` (Python 3.13) - the agent's execution runtime (default for agent-run code)

The agent installs packages into the execution runtime (`/opt/venv`) via `pip install`. Packages needed by the framework itself must target `/opt/venv-ctx0`.
...
- Project-scoped skills in `.ctx0proj/skills/`
...
- Custom agent instructions (`.ctx0proj/agent.instructions.md`)
...
CtxAI can act as both an MCP server and an MCP client:
