...
async def handle_start(message: TgMessage, bot_name: str, bot_cfg: dict):
    ...
    await _send_with_temp_bot(
        instance.bot.token,
        message.chat.id,
        f"\U0001f44b Hello {user.first_name}! I'm connected to CtxAI.\n\n"
    ...
    with _chat_map_lock:
        ...
        if ctx_id:
            ...
            if ctx:
                ctx.reset()
                PrintStyle.info(
                    f"Telegram ({bot_name}): cleared chat for user {user.id}"
                )
...
async def handle_message(message: TgMessage, bot_name: str, bot_cfg: dict):
    ...
    mq.log_user_message(
        context, user_msg, attachments, message_id=msg_id, source=" (telegram)"
    )
    context.communicate(
        UserMessage(
            message=user_msg,
...
async def handle_new_members(message: TgMessage, bot_name: str, bot_cfg: dict):
    ...
    for member in new_members:
        ...
        await _send_with_temp_bot(
            instance.bot.token, message.chat.id, text, parse_mode=None
        )
...
async def send_telegram_reply(
    ...
    try:
        async with _temp_bot(
            instance.bot.token, default=DefaultBotProperties(parse_mode=ParseMode.HTML)
        ) as reply_bot:
            if attachments:
                for path in attachments:
                    ...
                    if tc.is_image_file(local_path):
                        await tc.send_photo(
                            reply_bot, chat_id, local_path, reply_to_message_id=reply_to
                        )
                    else:
                        await tc.send_file(
                            reply_bot, chat_id, local_path, reply_to_message_id=reply_to
            ...
            if response_text:
                ...
                if keyboard:
                    ...
                    await tc.send_text(
                        reply_bot, chat_id, html_text, reply_to_message_id=reply_to
                    )
...
def _inherit_model_override(ctx: AgentContext):
    ...
    try:
        ...
        if not is_chat_override_allowed(ctx.ctx):
            return
    except Exception:
        return
    source = max(
