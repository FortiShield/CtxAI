...
//     if (out.name.startsWith("data-") && out.name != "data-CTX0UID" && out.name != "data-ctx0-frame-id") return null;
...
//   window.__CTX0_extractFrameContent = function() {
...
//           node.setAttribute("data-CTX0UID", uid);
//         }
...
//         if (tagName) {
//           content += ` selector="${node.getAttribute("data-CTX0UID")}"`;
//         }
...
//     if (event.data === 'CTX0_REQUEST_CONTENT') {
//       // Extract content and send it back to parent
//       const content = window.__CTX0_extractFrameContent();
//       // Use '*' as targetOrigin since we're in a controlled environment
//       window.parent.postMessage({
//         type: 'CTX0_FRAME_CONTENT',
//         content: content,
//         frameId: window.frameElement?.getAttribute('data-ctx0-frame-id')
//       }, '*');
//     }
//   });

//   // Function to extract content from all frames
//   window.__CTX0_extractAllFramesContent = async function(rootNode = document) {
//     let content = "";
    
//     // Extract content from current document
//     content += window.__CTX0_extractFrameContent();
...
//         iframe.setAttribute('data-ctx0-frame-id', frameId);
        
//         // Setup one-time message listener for this specific frame
//         const listener = function(event) {
//           if (event.data?.type === 'CTX0_FRAME_CONTENT' && 
...
//         iframe.contentWindow.postMessage('CTX0_REQUEST_CONTENT', '*');
...
//       const frameId = iframe.getAttribute('data-ctx0-frame-id');
