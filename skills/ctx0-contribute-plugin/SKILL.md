---
name: ctx0-contribute-plugin
description: Guide for publishing an CtxAI plugin to the community Plugin Index (ctx0-plugins repo). Covers GitHub repo setup, index.yaml creation, CI validation rules, and PR submission. Use when the user wants to share, publish, submit, or contribute a plugin to the Plugin Hub so other CtxAI users can find and install it.
version: 1.0.0
...
# CtxAI Plugin Contribution

This skill guides publishing a plugin to the [Plugin Index](https://github.com/ctxos/ctx-plugins), making it discoverable and installable by all CtxAI users.
...
1. Plugin exists and works locally in `/ctx0/usr/plugins/<name>/`
2. Plugin has been reviewed - if not, offer to run `ctx0-review-plugin` first:
...
gh repo create <repo-name> --public --description "CtxAI plugin: <title>"
git init
git add .
...
```
https://github.com/ctxos/ctx-plugins/releases/download/generated-index/index.json
```
...
```bash
# Automatic mode
gh repo fork https://github.com/ctxos/ctx-plugins --clone --remote
cd ctx0-plugins
...
Use tags from https://github.com/ctxos/ctx-plugins/blob/main/TAGS.md (up to 5).
...
|---|---|---|---|
| `plugin.yaml` | Your plugin's GitHub repo root | Runtime manifest (drives CtxAI behavior) | `name` (required!), `title`, `description`, `version`, `settings_sections`, `per_project_config`, `per_agent_config`, `always_enabled` |
| `index.yaml` | `ctx0-plugins/plugins/<name>/` | Index manifest (drives discoverability) | `title`, `description`, `github`, `tags`, `screenshots` |
...
- Plugin Index repo: https://github.com/ctxos/ctx-plugins
- Recommended tags: https://github.com/ctxos/ctx-plugins/blob/main/TAGS.md
- Review before contributing: read `/ctx0/skills/ctx0-review-plugin/SKILL.md`
- Build the plugin first: read `/ctx0/skills/ctx0-create-plugin/SKILL.md`
