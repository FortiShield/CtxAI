---
name: ctx0-development
description: Development guide for extending and building features for the CtxAI AI framework. Covers architecture, tools, extensions, API endpoints, agent profiles, projects, prompts, and skills — with correct paths, imports, and patterns matching the current codebase.
version: 1.0.0
author: CtxAI Team
tags: ["development", "framework", "ctxai", "extending", "tools", "extensions", "skills", "api", "agents", "prompts"]
trigger_patterns:
  - "extend ctxai"
  - "ctxai development"
  - "build ctxai feature"
...
# CtxAI Development Guide

This skill provides comprehensive, accurate guidance for extending and building features for CtxAI. Use it when you need to:
...
> **Path convention:** Throughout this guide, `/ctx0/` refers to the framework root — this is `/ctx0/` inside Docker, or your local repository root in development. All paths are relative to this root.

> [!IMPORTANT]
> **Plugins are the primary way to extend CtxAI.** Most new tools, extensions, and prompts should be packaged as plugins. For all plugin tasks (create, review, manage, debug, contribute), load the `ctx0-plugin-router` skill which routes to the appropriate specialist. This guide covers the underlying framework patterns that plugins build upon.

Related skills: `ctx0-plugin-router` (plugin tasks) | `create-skill` (skill creation wizard) | `ctx0-create-plugin` | `ctx0-review-plugin` | `ctx0-manage-plugin` | `ctx0-contribute-plugin` | `ctx0-debug-plugin`
...
1. **Plugin-first design** — Most capabilities (tools, extensions, prompts) are delivered via plugins in `/ctx0/plugins/` (core) or `/ctx0/usr/plugins/` (user).
...
8. **User-space separation** — Everything under `/ctx0/usr/` survives framework updates.
...
| Location | Purpose |
|---|---|
| `/ctx0/tools/` | Core framework tools (search, response, call_subordinate, etc.) |
| `/ctx0/plugins/<plugin>/tools/` | Plugin-provided tools (code_execution, memory, text_editor) |
| `/ctx0/agents/<profile>/tools/` | Profile-specific tool overrides |
| `/ctx0/usr/plugins/<plugin>/tools/` | User plugin tools |

### Example: Creating a Tool

Based on the actual `_example` profile in `/ctx0/agents/_example/tools/example_tool.py`:
...
| Source | Path |
|---|---|
| Core extensions | `/ctx0/extensions/python/<hook_point>/` |
| Plugin extensions | `/ctx0/plugins/<plugin>/extensions/python/<hook_point>/` |
| User extensions | `/ctx0/usr/extensions/python/<hook_point>/` |
| Agent profile extensions | `/ctx0/agents/<profile>/extensions/<hook_point>/` |
| User plugin extensions | `/ctx0/usr/plugins/<plugin>/extensions/python/<hook_point>/` |
...
Based on the actual `_example` profile in `/ctx0/agents/_example/extensions/agent_init/_10_example_extension.py`:
...
| Location | Purpose |
|---|---|
| `/ctx0/api/` | Core API endpoints |
| `/ctx0/plugins/<plugin>/api/` | Plugin API endpoints |
| `/ctx0/usr/plugins/<plugin>/api/` | User plugin API endpoints |
...
| Location | Purpose |
|---|---|
| `/ctx0/agents/` | Core profiles (default, agent0, developer, hacker, researcher) |
| `/ctx0/usr/agents/` | User-created profiles (survives updates) |
...
```yaml
# /ctx0/usr/agents/data-analyst/agent.yaml
title: Data Analyst
...
```markdown
<!-- /ctx0/usr/agents/data-analyst/prompts/agent.system.main.role.md -->
...
The framework includes a complete example profile at `/ctx0/agents/_example/` that demonstrates:
- Custom tool: `/ctx0/agents/_example/tools/example_tool.py`
- Custom extension: `/ctx0/agents/_example/extensions/agent_init/_10_example_extension.py`
- Tool prompt: `/ctx0/agents/_example/prompts/agent.system.tool.example_tool.md`
- Role prompt: `/ctx0/agents/_example/prompts/agent.system.main.role.md`

---

## Prompt System

CtxAI assembles system prompts from **named fragments** using includes and variable substitution.
...
|---|---|---|
| `/ctx0/agents/<profile>/prompts/` | Highest | Profile-specific overrides |
| `/ctx0/usr/agents/<profile>/prompts/` | High | User profile overrides |
| `/ctx0/plugins/<plugin>/prompts/` | Normal | Plugin-provided prompts |
| `/ctx0/usr/plugins/<plugin>/prompts/` | Normal | User plugin prompts |
| `/ctx0/prompts/` | Base | Core framework prompts |
...
| Location | Purpose |
|---|---|
| `/ctx0/skills/` | Core skills (shipped with framework) |
| `/ctx0/usr/skills/` | User-created skills (survives updates) |
...
> Projects are typically created and managed via the Web UI. The `.ctx0proj/` directory and `project.json` are auto-generated when you create a project through the UI.

### Project Structure

```
/ctx0/usr/projects/<project-name>/
+-- .ctx0proj/
...
Plugins are the **primary extension mechanism** in CtxAI. A plugin can bundle tools, extensions, prompts, API endpoints, helpers, and UI components into a self-contained package.

> For all plugin tasks — creating, reviewing, managing, contributing, or debugging plugins — load the `ctx0-plugin-router` skill, which routes to the appropriate specialist skill.

### Core Plugins

The framework ships with these core plugins in `/ctx0/plugins/`:
...
When building features for CtxAI:
...
- New plugins -> `/ctx0/usr/plugins/<name>/`
- New profiles -> `/ctx0/usr/agents/<name>/`
- New skills -> `/ctx0/usr/skills/<name>/`
- New extensions -> `/ctx0/usr/extensions/python/<hook_point>/`
...
For contribution guidelines, see `/ctx0/docs/contribution.md`. For plugin contributions to the community Plugin Index, load the `ctx0-contribute-plugin` skill.
...
### DO
- Use the **plugin system** for new features (see `ctx0-create-plugin` skill)
...
- Develop in `/ctx0/usr/` directories to survive updates
- Test with the `_example` profile as a reference
- Use `from helpers.*` imports (not `from python.helpers.*`)

### DON'T
- Modify files in `/ctx0/plugins/` or `/ctx0/tools/` directly (use usr/ space)
...
| File | Purpose |
|---|---|
| `/ctx0/agent.py` | Core `Agent`, `AgentContext`, `AgentConfig` classes |
| `/ctx0/helpers/tool.py` | `Tool` + `Response` base classes |
| `/ctx0/helpers/extension.py` | `Extension` base + `@extensible` decorator |
| `/ctx0/helpers/api.py` | `ApiHandler` base class |
| `/ctx0/helpers/files.py` | File ops + prompt reading |
| `/ctx0/helpers/plugins.py` | Plugin system manager |
| `/ctx0/helpers/print_style.py` | Console output formatting |
| `/ctx0/agents/_example/` | Reference example profile with tool + extension |
| `/ctx0/prompts/agent.system.main.md` | Main system prompt entry point |