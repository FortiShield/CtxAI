...
ls /ctx0/usr/plugins/<name>/plugin.yaml

# Validate YAML
python3 -c "import yaml; yaml.safe_load(open('/ctx0/usr/plugins/<name>/plugin.yaml'))"
...
```bash
# Check toggle state
ls -la /ctx0/usr/plugins/<name>/.toggle-*

# Check for conflicting scoped toggles
ls -la project/.ctx0proj/plugins/<name>/.toggle-* 2>/dev/null
ls -la /ctx0/usr/agents/default/plugins/<name>/.toggle-* 2>/dev/null
```
...
- Check for Python import errors on startup (check CtxAI logs)
...
python3 -m py_compile /ctx0/usr/plugins/<name>/api/my_handler.py
```
...
find /ctx0 -path "*/plugins/<name>/config.json" 2>/dev/null
```
...
- Manually trigger in the **framework runtime** (not via `code_execution_tool` python - that uses `/opt/venv`, not `/opt/venv-ctx0`):

```bash
cd /ctx0 && /opt/venv-ctx0/bin/python -c "
import asyncio
...
1. CtxAI walks `usr/plugins/` then `plugins/` at startup
...
- Manage (install/update/uninstall): read `/ctx0/skills/ctx0-manage-plugin/SKILL.md`
