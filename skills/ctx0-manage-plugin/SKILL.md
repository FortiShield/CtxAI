---
name: ctx0-manage-plugin
description: Manage CtxAI plugins lifecycle: browse the Plugin Hub, scan for security, install from Git/ZIP/Plugin Hub, update, uninstall, enable, disable, debug, and troubleshoot. Use when asked to install, update, uninstall, remove, scan, find, search, enable, disable, debug, or troubleshoot a plugin.
version: 1.2.0
...
| Plugin not loading / crashing / missing | Read `/ctx0/skills/ctx0-debug-plugin/SKILL.md` |
| Explain how plugin discovery works | Read `/ctx0/skills/ctx0-debug-plugin/SKILL.md` |
...
url = "https://github.com/ctxos/ctx-plugins/releases/download/generated-index/index.json"
...
**Alternatively via UI**: Open the Plugins dialog in CtxAI and switch to the **Browse** tab (or click **Install** in the toolbar to open the Plugin Hub).
...
> "Before installing, I strongly recommend running a security scan on this plugin. Third-party plugins execute code inside your CtxAI environment. Should I scan it first? The scan typically takes 2-4 minutes."
...
> **Always use the HTTP API or UI.** Never import CtxAI framework modules directly from `code_execution_tool` - the agent runs in a separate Python runtime (`/opt/venv`) that does not have the framework's dependencies. All programmatic installs must go through HTTP.
...
The CtxAI API uses CSRF protection. The `Origin` header is **always required** - without it the CSRF endpoint returns `ok: false` even when login is disabled.

**Step 1: Set the base URL.** CtxAI listens on port 80 inside Docker (the standard deployment):
...
```bash
git clone https://github.com/<user>/<repo> /ctx0/usr/plugins/<plugin_name>
```
...
Or simply restart CtxAI - on startup it re-scans `usr/plugins/` fresh.
...
git -C /ctx0/usr/plugins/<name> rev-parse --is-inside-work-tree 2>/dev/null

# Compare local HEAD with remote HEAD (no fetch required)
LOCAL=$(git -C /ctx0/usr/plugins/<name> rev-parse HEAD)
REMOTE=$(git -C /ctx0/usr/plugins/<name> ls-remote origin HEAD | awk '{print $1}')
echo "Local:  $LOCAL"
echo "Remote: $REMOTE"
...
ls /ctx0/usr/plugins/<name>/

# Remove it
rm -rf /ctx0/usr/plugins/<name>/
```

After manual removal, refresh the plugin list via the UI or restart CtxAI.
...
**Enable a plugin**:
```bash
rm -f /ctx0/usr/plugins/<name>/.toggle-0
touch /ctx0/usr/plugins/<name>/.toggle-1
```

**Disable a plugin**:
```bash
rm -f /ctx0/usr/plugins/<name>/.toggle-1
touch /ctx0/usr/plugins/<name>/.toggle-0
```
...
- Project scope: `project/.ctx0proj/plugins/<name>/.toggle-1`
...
- Plugin architecture: `/ctx0/docs/agents/AGENTS.plugins.md`
- Developer lifecycle guide: `/ctx0/docs/developer/plugins.md`
- Debug a broken plugin: read `/ctx0/skills/ctx0-debug-plugin/SKILL.md`
- Create a new plugin: read `/ctx0/skills/ctx0-create-plugin/SKILL.md`
- Review a plugin: read `/ctx0/skills/ctx0-review-plugin/SKILL.md`
