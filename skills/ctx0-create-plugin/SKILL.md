---
name: ctx0-create-plugin
description: Create, extend, or modify CtxAI plugins. Follows strict full-stack conventions (usr/plugins, plugin.yaml, Store Gating, AgentContext, plugin settings). Use for UI hooks, API handlers, lifecycle extensions, or plugin settings UI.
version: 1.0.0
...
# CtxAI Plugin Development

> [!IMPORTANT]
> Always create new plugins in `/ctx0/usr/plugins/<plugin_name>/`. The `/ctx0/plugins/` directory is reserved for core system plugins.

Related skills: `/ctx0/skills/ctx0-review-plugin/SKILL.md` | `/ctx0/skills/ctx0-contribute-plugin/SKILL.md` | `/ctx0/skills/ctx0-manage-plugin/SKILL.md`

Primary references:
- /ctx0/AGENTS.md (Full-stack architecture & AgentContext)
- /ctx0/docs/agents/AGENTS.components.md (Component system deep dive)
- /ctx0/docs/agents/AGENTS.modals.md (Modal system & CSS conventions)
- /ctx0/docs/agents/AGENTS.plugins.md (Extension points, plugin.yaml, settings system, Plugin Index)
- /ctx0/docs/developer/plugins.md (Developer lifecycle and publishing)
...
> "Should this plugin be **local only** (stays in your CtxAI installation) or a **community plugin** (published to the Plugin Index so others can install it)?"

- **Local plugin**: Create it in `/ctx0/usr/plugins/<plugin_name>/`. No repository needed. Skip to the manifest section below.
...
Import and call from your store; do not render a dedicated error/success block in the template. See [Notifications](/ctx0/docs/developer/notifications.md) for the full API.
...
- `hooks.py` runs inside the **CtxAI framework runtime**, not the separate agent execution environment.
...
- If `hooks.py` runs `sys.executable -m pip install ...`, it installs into the same Python environment that is running CtxAI.
...
In Docker, this usually means `hooks.py` affects `/opt/venv-ctx0` unless you intentionally target `/opt/venv` or another environment.
...
The Plugin Index (`https://github.com/ctxos/ctx-plugins`) uses a **separate `index.yaml`** file that only describes discoverability — it is NOT the same as the runtime `plugin.yaml` and has a different schema:
...
See the recommended tag list at https://github.com/ctxos/ctx-plugins/blob/main/TAGS.md.
...
For a fully guided contribution flow (including git operations), read `/ctx0/skills/ctx0-contribute-plugin/SKILL.md`.
...
CtxAI now exposes indexed plugins through the built-in **Plugin Hub**. Users can open it from the **Plugins** dialog either through the **Browse** tab or through the **Install** button, then inspect plugin details and install directly from the UI.
